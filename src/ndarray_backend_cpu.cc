  ...
  while (cnt < total_size) {
                           ...
                           float *__restrict__ out)
    {
      ...
      for (size_t i = 0; i < TILE; ++i)
      {
        ...
        {
          a0 = &a[i * TILE];         // pointer to the current row of a
          c0 = out[i * TILE + j];    // current value of out
          c0 += *a0++ * b[j];        // multiply and add to c0
          c0 += *a0++ * b[TILE + j]; // multiply and add to c0
          c0 += *a0++ * b[TILE * 2 + j];
          c0 += *a0++ * b[TILE * 3 + j];
          c0 += *a0++ * b[TILE * 4 + j];
          c0 += *a0++ * b[TILE * 5 + j];
                     ...
                     uint32_t n, uint32_t p)
    {
      ...
      float *A = new float[TILE * n]; // 4*2 = 8
      float *B = new float[TILE * n]; // 4*2 = 8
      float *C = new float[TILE2];    // 4
      ...
      { // iterates 2 times
        ...
        { // this iterates over columns of A
