...
class NDArray:
    """A generic ND array class that supports multiple backends
    (NumPy, native CPU, or GPU).

    Supports float32 precision for numerical computation.
    """
    ...
    def __array__(self, dtype=None):
        """Support numpy conversion via np.array(ndarray)."""
        arr = self.numpy()
    ...
    def reshape(self, new_shape):
        ...
        return arr.as_strided(_shape, _strides)
    def permute(self, new_axes):
        ...
        _shape, _strides = tuple(_shape), tuple(_strides)
        return self.as_strided(_shape, _strides)
    def swapaxes(self, axis1, axis2):
        """
        Swap two axes of the array.  This is equivalent to .permute() with the
        two axes swapped, so .swapaxes(1,2) would be equivalent to .permute((0,2,1))
        ...
        Returns:
            NDArray: new NDArray object with the two axes swapped, pointing to
            the same memory as the original NDArray.
        """
        _shape = list(self.shape)
        _strides = list(self.strides)
        _shape[axis1], _shape[axis2] = _shape[axis2], _shape[axis1]
        _strides[axis1], _strides[axis2] = _strides[axis2], _strides[axis1]
        return self.as_strided(tuple(_shape), tuple(_strides))
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(new_shape)):
            if padded_shape[i] == 1:
                ...
                assert padded_shape[i] == new_shape[i]
                _strides.append(padded_strides[i])
        return self.as_strided(new_shape, tuple(_strides))
    ...
    def __getitem__(self, idxs):
        ...
        if len(idxs) < self.ndim:
            idxs = idxs + tuple(slice(0, s, 1) for s in self.shape[len(idxs):])
        _shape = []
        _strides = []
    ...
    def __pow__(self, other):
        if isinstance(other, NDArray):
            return NDArray(np.power(self.numpy(), other.numpy()), device=self.device)
        out = NDArray.make(self.shape, device=self.device)
    ...
    ### This prepares the view and output for the reduction function.
    ### View => axis of interest sent to end , Out => axis of interest reduced to 1
    ...
    def sum(self, axis=None, keepdims=True):
        ...
        if isinstance(axis, (tuple, list)) and len(axis) > 1:
            ...
            for ax in sorted(axis, reverse=True):
                result = result.sum(ax, keepdims=keepdims)
            return result
    ...
    def max(self, axis=None, keepdims=True):
        ...
        if isinstance(axis, (tuple, list)) and len(axis) > 1:
            result = self
            for ax in sorted(axis, reverse=True):
                result = result.max(ax, keepdims=keepdims)
            return result
        view, out = self.reduce_view_out(axis, keepdims=keepdims)
    ...
    def squeeze(self):
        """
        Remove all 1 dimensions from the array.
        """
        shape = [i for i in self.shape if i != 1]
...
def _to_numpy(a):
    """Convert NDArray or numpy array to numpy array."""
    return a.numpy() if hasattr(a, '_handle') else np.asarray(a)


def _from_numpy_or_passthrough(result, ref):
    """Convert result to NDArray if ref is NDArray, otherwise return as-is."""
    if isinstance(ref, np.ndarray):
        return result
    return NDArray(result, device=ref.device)


def stack(arrays, axis):
    output = np.stack(tuple(_to_numpy(i) for i in arrays), axis=axis)
    if isinstance(arrays[0], np.ndarray):
        return output
    return NDArray(output, device=arrays[0].device)

def split(a, axis):
    num_splits = a.shape[axis]
    new_shape = list(a.shape)
    new_shape.pop(axis)
    a_np = _to_numpy(a)
    output = np.split(a_np, num_splits, axis=axis)
    if isinstance(a, np.ndarray):
        return tuple(i.reshape(new_shape) for i in output)
    return tuple(NDArray(i, device=a.device).reshape(new_shape) for i in output)
...
def dilate(a, axes, dilation):
    ...
    for ax in axes:
        if ax >= len(out_shape):
            continue
        out_shape[ax] = out_shape[ax] * (1 + dilation)
        slices[ax] = slice(0, out_shape[ax], 1 + dilation)

    out_tensor = np.zeros(out_shape, dtype=a_numpy.dtype)
    out_tensor[tuple(slices)] = a_numpy

    return _from_numpy_or_passthrough(out_tensor, a)
...
def undilate(a, axes, dilation):
    ...
    for ax in axes:
        if ax >= len(shape):
            continue
        slices[ax] = slice(0, shape[ax], 1 + dilation)

    return _from_numpy_or_passthrough(a_numpy[tuple(slices)], a)
